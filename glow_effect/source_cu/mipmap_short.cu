...
#define SHORT_UNIT  (1<<15)
#define SHORT_MASK  (SHORT_UNIT-1)
...
d_gen_mipmap(cudaSurfaceObject_t mipOutput, cudaTextureObject_t mipInput, uint imageW, uint imageH)
{
	...
	if ((x < imageW) && (y < imageH))
	{
		...
		color /= 4.0f;
		color *= SHORT_MASK;
		color = fminf(color, make_float4(255.f));

		surf2Dwrite(to_short4(color), mipOutput, x * sizeof(T), y);
	}
}
...
gen_mipmap(cudaMipmappedArray_t& mipmapArray, cudaExtent size)
{
	...
	while (width != 1 || height != 1)
	{
		...
		d_gen_mipmap<short4> << <gridSize, blockSize >> > (surfOutput, texInput, (uint)width, (uint)height);
...
__global__ void
d_get_mipmap(cudaTextureObject_t texEngine, const int width, const int height, const float* lod, short4* dout)
{
	...
	if (xi < width && yi < height)
	{
		...
		data = fminf(SHORT_MASK * data, make_float4(255.f));
		data = fmaxf(data, make_float4(0.f));
		dout[idx] = to_short4(data);
	}
}

__global__ void
d_get_mipmap(cudaTextureObject_t texEngine, const int width, const int height, const float scale, short4* dout)
{
	int xi = blockIdx.x * blockDim.x + threadIdx.x;
	int yi = blockIdx.y * blockDim.y + threadIdx.y;
	int idx = yi * width + xi;
...
static void
get_mipmap(cudaMipmappedArray_t mm_array, const int3 img_size, const float scale, short4* dout)
{
	...
	*/
	short4* d_out;
	checkCudaErrors(cudaMalloc(&d_out, asize * sizeof(short4)));
	...
	checkCudaErrors(cudaMemcpy(dout, d_out, asize * sizeof(short4), cudaMemcpyKind::cudaMemcpyDeviceToHost));
...
void filter_mipmap(const int width, const int height, const float scale, const short4* src_img, short4* dst_img)
{
	...
	cudaChannelFormatDesc ch_desc = cudaCreateChannelDesc<short4>();
	...
	cpy_param.srcPtr = make_cudaPitchedPtr((void*)src_img, width * sizeof(short4), width, height);
...
void test_mipmap_short(void)
{
	...
	uchar4* uchar4_img = new uchar4[asize];
	for (int i = 0, m = 0; i < height; i++, m += width) {
		for (int j = 0, n = m; j < width; j++, n++) {
			uchar4_img[n].x = j + 1;
			uchar4_img[n].y = i + 1;
			uchar4_img[n].z = i + j + 1;
			uchar4_img[n].w = 255;
		}
	}
#else
	...
	sdkLoadPPM4(imgPath, (unsigned char**)&uchar4_img, &width, &height);
#endif

	for (int k = 0; k < 5; k++)
		button_State[k] = true;
	short4* src_img = new short4[width * height];
	short4* dst_img = new short4[width * height];
	for (uint i = 0, m = 0; i < height; i++, m += width) {
		for (uint j = 0, n = m; j < width; j++, n++) {
			src_img[n].x = uchar4_img[n].x;
			src_img[n].y = uchar4_img[n].y;
			src_img[n].z = uchar4_img[n].z;
			src_img[n].w = uchar4_img[n].w;
		}
	}

	filter_mipmap(width, height, blur_scale, src_img, dst_img);
	...
	for (uint i = 0, m = 0; i < height; i++, m += width) {
		for (uint j = 0, n = m; j < width; j++, n++) {
			fputc(min(255, max(0, dst_img[n].x)), fp);
			fputc(min(255, max(0, dst_img[n].y)), fp);
			fputc(min(255, max(0, dst_img[n].z)), fp);
		}
	}
	fclose(fp);
	delete[] src_img;
